...
| 12 | 供电 | **5V 3A 充放电一体模块 (Type-C 接口)** | **5V 3A 同步升压 + Type-C 锂电充电管理一体** | 1 | 10 元 | 集成 104050 电池充电与 5V 3A 升压输出，机身直出 Type-C 口，免额外加充电机板。搜索词：`5V 3A 充放电一体模块 Type-C`。 |
| 13 | 电池 | **104050 聚合物软包锂电池 (已购入)** | **3.7V 2800mAh 带保护板 (10×40×50mm)** | 1 | 14 元 | **实测确认款**：超薄 10mm 完美塞进狗腹，大幅降低整机重心，2800mAh 续航超 2 小时。 |
...
### 4. 5V 3A 充放电一体模块 (Type-C 接口)
* **搜索关键词**：`5V 3A 充放电一体模块 Type-C` 或 `Type-C 锂电池 5V 3A 充放电一体板`
* **必认指标**：
  * **充放一体优势**：同时集成锂电池充电管理（Type-C 充入）与 5V 3A 同步升压放电（给 4 舵机和单片机供电），机身只需露出一个 Type-C 口即可随时充电或边充边用，彻底省去单独加装 TP4056 充电板的体积与飞线；
  * **核心防踩坑（轻载自动关机）**：必须向卖家客服确认该模块**无轻载自动断电关机机制**（或支持常开模式），避免小狗静止站立时被当作拔出手机而自动切断 5V 输出；
  * **电流核对**：板载大号贴片屏蔽电感，标称持续输出电流必须达到 **3A**，禁止使用额定仅 2A 的缩水板。

### 5. 电池选型确认：104050 2800mAh 聚合物软包锂电池 (已实测拍定)
* **最终确定型号**：`104050`（厚 10mm × 宽 40mm × 长 50mm），容量 **2800mAh**，带充放电保护板。
* **选型优势**：
  * **超薄低重心**：相比圆柱形 18650（直径 18mm、长 65mm，塞进桌面狗腹部极其拥挤甚至卡腿），104050 厚度仅 10mm，底盘平整且重心极低，小狗小跑完全不侧翻；
  * **续航超神**：2800mAh 充沛容量，实测支撑 4 路舵机持续走动 + ESP32-S3 大模型对话约 **2 ~ 2.5 小时**；
  * **安装防护守则**：软包铝塑膜怕被尖锐物扎伤，3D 打印件内壁若有螺丝尖角或毛刺，务必贴一层附赠的 **EVA 吸音/减震泡棉** 或纤维胶带作物理隔绝保护。
...
timeline
    ...
    Phase 3 (动力闭环) : 5V 3A 充放电一体模块 Type-C (10元) : 104050 2800mAh软包电池(已购14元) : 1000µF 滤波电容 : 硅胶特软线 (预算约 32 元)
```
